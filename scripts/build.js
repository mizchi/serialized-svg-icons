...
async function getIconFiles(content) {
  const ret = await glob(content.files);
  console.log("glob result", content.files);
  return ret;
}
...
async function dirInit() {
  ...
  const gitignore =
    ["# autogenerated", ...icons.map((icon) => `/${icon.id}/`)].join("\n") +
    "\nREADME.md\n\n";
  await fs.writeFile(path.resolve(DIST, ".gitignore"), gitignore);
...
async function writeIconModule(icon) {
  ...
  for (const content of icon.contents) {
    ...
    for (const file of files) {
      ...
      console.log("append", path.resolve(DIST, icon.id, "index.js"));
...
async function writeEntryPoints() {
  await fs.writeFile(path.resolve(DIST, "index.js"), "", "utf8");
}
